white_pawn_available_moves <- function(board, piece) {
  ...
  ### check for attack
  if (piece@col != "h") {
    if (board[[letters[which(letters == piece@col) + 1]]][[piece@row + 1]]@color == "black") {
      piece@available_moves <- piece@available_moves |>
        append(list(list(
          col = letters[which(letters == piece@col) + 1],
          row = piece@row + 1
        )))
    }
  }
...
black_pawn_available_moves <- function(board, piece) {
  ...
  ### check for attack
  if (piece@col != "h") {
    if (board[[letters[which(letters == piece@col) + 1]]][[piece@row - 1]]@color == "white") {
      piece@available_moves <- piece@available_moves |>
        append(list(list(
          col = letters[which(letters == piece@col) + 1],
          row = piece@row - 1
